...
2. **Filtro date default:** ultimi 30 giorni (riduce carico cognitivo su archivi lunghi)
   - Mostra "Dal: 11/12/2025 Al: 10/01/2026"
   - Pulsante "Mostra tutto" → espande a tutte le riparazioni in corso (dalla più vecchia incompleta a oggi)
   - Date modificabili manualmente con input formattati (digitando "0101" → "01/01/____" automatico)
3. **Contatori dinamici sui toggle:**
   - "Tutti (45)" → totale riparazioni nel range date selezionato
   - "Da completare (18)" → riparazioni in corso nel range
   - Contatori si aggiornano in tempo reale al cambio filtri
4. Toggle "Da completare" → mostra solo riparazioni in corso
5. Ricerca per nome cliente (debounce 300ms, filtro client-side)
   - Contatori si aggiornano anche con la ricerca
6. **Tabella a 5 colonne:**
   - Desktop: Numero (15%) | Data ins. (15%) | Data com. (15%) | Cliente (40%) | Stato (15%)
   - Mobile: Num. | Ins. | Com. | Cliente | Stato (emoji 🔴🟢)
   - Data completamento mostra "-" se scheda non completata o completata prima dell'implementazione
7. Tocca riga → `/html/riparazioni-dettaglio?numero=25/0042`
8. Visualizza dettaglio completo (cliente, telefono, attrezzi, stato, **data completamento se presente**)
9. Navigazione prev/next istantanea (usa cache in memoria, no reload)
10. "Modifica scheda" → form pre-compilato
11. Cambia stato da "In corso" a "Riparato" → **data completamento salvata automaticamente** (backend GMT+1)
12. **Dati aggiornati localmente** (no API call per reload)
13. Burger menu ☰ → torna ad archivio

**Performance target:**
- Prima apertura archivio: 2-3s (cache scaduta)
- Aperture successive (<15 min): 0.1s istantaneo
- Navigazione prev/next: 0s (no reload)
- Filtro date: calcolo istantaneo client-side (<0.05s)
